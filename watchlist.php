...
<?php 
 ...
 if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $nome4 = $_POST["nome4"];
        ...
        $watchlist = "$nomePerfil,$nome1,$nome2,$nome3,$nome4";
...
<header>
  ...
  echo $_SESSION['nome'];    ?>
    ...
    <?php 
        $foo=FALSE;
        $favoritos = file_exists("watchlist.txt") ? file("watchlist.txt", FILE_IGNORE_NEW_LINES) : [];
        
        foreach ($favoritos as $linha) {
        $dados = explode(",", $linha);
        if ($dados[0] == $nomePerfil) {
            $foo=TRUE;
            echo "<h2>Filmes na watchlist</h2>";
            echo "<p>";
            echo $dados[1];
         ...
         if($foo == false){
            ...
            ?>
        <p>Por favor digite 4 filmes que você desseja assistir</p>
